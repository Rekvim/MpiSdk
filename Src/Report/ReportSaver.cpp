...
{
    if (!m_registry) {
        qCritical() << "ReportSaver: Registry is not set";
        emit Question("Ошибка", "Сервис данных не инициализирован. Повторите после выбора объекта.", /*out*/ *(new bool(false)));
        // Можно выбрать дефолтную директорию, чтобы не падать:
        m_dir.setPath(QCoreApplication::applicationDirPath());
        m_created = m_dir.mkpath("Reports") && m_dir.cd("Reports");
        return;
    }
