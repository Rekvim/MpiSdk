...
bool ReportSaver::SaveReport(const Report &report)
{
    ...
    for (const auto &data : report.data) {
        qDebug() << data.x << data.y << data.text;
        xlsx.write(data.x, data.y, data.text);
    }

    if (!report.image1.isNull()) {
        xlsx.insertImage(84, 1, report.image1);
    }

    if (!report.image2.isNull()) {
        xlsx.insertImage(109, 1, report.image2);
    }

    if (!report.image3.isNull()) {
        xlsx.insertImage(134, 1, report.image3);
    }
